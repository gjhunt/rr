...
svd_na <- function(X, nu = NULL, nv = NULL) {
    X <- as.matrix(X)
    eU <- eigen(gram(X, chir = "left", na = 0),symmetric=TRUE)
    eV <- eigen(gram(X, chir = "right", na = 0),symmetric=TRUE)
    eU$vectors = eU$vectors[,c(sort(which(eU$values>0)),sort(which(eU$values<0)),sort(which(eU$values==0)))]
    eU$values = sapply(eU$values,max,0)
    eV$vectors = eV$vectors[,c(sort(which(eV$values>0)),sort(which(eV$values<0)),sort(which(eV$values==0)))]
...
gram <- function(A, chir = "left", na = NA,
                 project=TRUE) {
    if (chir == "right") 
        A <- t(A)
    gram <- A %:% t(A)
